...
INSTALLDIR=/Users/Dariusz/gits/bg-wordpress/wordpress-4.7-0
...
help() {
    ...
    if test -x $ZOOKEEPER_SCRIPT; then
	    echo "       $0 (start|stop|restart|status) zookeeper"
	fi
    if test -x $KAFKA_SCRIPT; then
	    echo "       $0 (start|stop|restart|status) kafka"
	fi
...
if [ "x$1" = "xhelp" ] || [ "x$1" = "x" ]; then
    ...
    elif [ "x$2" = "xzookeeper" ]; then
        if test -x $ZOOKEEPER_SCRIPT; then
            $ZOOKEEPER_SCRIPT start
            ZOOKEEPER_ERROR=$?
        fi
    elif [ "x$2" = "xkafka" ]; then
        if test -x $KAFKA_SCRIPT; then
            $KAFKA_SCRIPT start
            KAFKA_ERROR=$?
        fi
...
# Checking for errors
for e in $VARNISH_ERROR $PHPFPM_ERROR $HHVM_ERROR $APACHE_ERROR $NGINX_ERROR $MYSQL_ERROR $MONGODB_ERROR $CASSANDRA_ERROR $COUCHDB_ERROR $SUBVERSION_ERROR $SOLR_ERROR $ELASTICSEARCH_ERROR $LOGSTASH_ERROR $LOGSTASHFORWARDER_ERROR $KIBANA_ERROR $TOMCAT_ERROR $REDIS_ERROR $RESIN_ERROR $MEMCACHED_ERROR $INGRES_ERROR $LIBREOFFICE_ERROR $LUCENE_ERROR $ZOPE_ERROR $POSTGRESQL_ERROR $THIRD_ERROR $NAGIOS_ERROR $RABBITMQ_ERROR $ACTIVEMQ_ERROR $ZOOKEEPER_ERROR $KAFKA_ERROR $JETTY_ERROR $JBOSS_ERROR $WILDFLY_ERROR $HYPERSONIC_ERROR; do
    if [ $e -gt 0 ]; then
        ERROR=$e
    fi
done
